...
int16_t shAMG_PUB_CMN_CalcAve( int16_t* pshArray, USHORT usSize, UCHAR ucSkip, UCHAR ucMedian, BOOL* pbMedianWork )
{
	...
	if( 0 == ucMedian )
	{
		...
		{
			int16_t	shCurData = pshArray[usCnt * ucSkip];
			loSum += shCurData;
		}
		shAve = (int16_t)(loSum / usSize);
	}
	else
	{
		USHORT	usCnt = 0;
		...
		{
			int16_t	shMaxData = SHORT_MIN_VAL;
			int16_t	shMinData = SHORT_MAX_VAL;
			UCHAR	ucIndex = 0;
		...
		shAve = (int16_t)(loSum / ( usSize - ucMedian * 2 ));
	}
...
{
	const int16_t c_shMinTh = 0;
	const int16_t c_shMaxTh = 256;
	long loAddVal = 0;
	...
	return( shVal1 + (int16_t)(loAddVal / c_shMaxTh) );
}
...
{
	{
		...
		for( ucY = 0; ucY < ucHeight; ucY++ )
		{
			for( ucX = 0; ucX < (ucWidth+1)/2; ucX++ )
			{
				...
				int16_t	shVal = pshInImg[usIndex1];
...
BOOL bAMG_PUB_IMG_LinearInterpolation( UCHAR ucWidth, UCHAR ucHeight, int16_t* pshInImg, int16_t* pshOutImg )
{
	...
	if( pshInImg != pshOutImg )
	{
		...
		for( usImg = 0; usImg < ucWidth * ucHeight; usImg++ )
		{
			...
			pshOutImg[usImg] = (int16_t)loWork;
		}
...
BOOL bAMG_PUB_BGT_UpdateBackTemp( USHORT usSize, UCHAR* pucImg, int16_t* pshDiffImg, int16_t shTh, int16_t* pshBackImg )
{
	...
	if( FALSE != bAMG_PUB_FEA_CalcAveTemp( usSize, 0, pucImg, pshDiffImg, &shAve ) )
	{
		...
		for( usImg = 0; usImg < usSize; usImg++ )
		{
			...
			if( 0 == pucImg[usImg] )
			{
				shTemp = (int16_t)( (long)shTh * pshDiffImg[usImg] / c_shMaxTh );
			}
			else
			{
				shTemp = (int16_t)( (long)shTh * shAve             / c_shMaxTh );
			}
